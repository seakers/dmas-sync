...
from factory import get_base_path, load_templates, generate_scenario_mission_specs
...
def generate_medium_latency_connectivity_spec_GA(imaging_sats : list, 
                                              relay_sats : list, 
                                              ground_stations : list, 
                                              high_latency_values : dict, 
                                              low_latency_values : dict,
...
def build_connectivity_index(scenario_orbitdata: Dict[str, "OrbitData"]) -> ConnectivityIndex:
    ...
    # Pull all rows once
    rows = list(orbitdata.comms_links.iter_rows_raw_fast(t=0.0, include_current=True))
    K = len(rows)
    if K == 0:
...
def evaluate_scenario_latency(mission_specs: dict,
    ...
    # close all orbitdata files to free resources
    for od in scenario_orbitdata.values():
        od.close()
...
if __name__ == "__main__":
    ...
    # load trial csv
    trials_filename = "full_factorial_trials_2026-05-01.csv"
    trials_path = os.path.join(".", 'experiments','2_centralized_vs_decentralized', 'resources', "trials", trials_filename)
    ...
    for _, row in trials_df.iterrows():
        ...
        sats_results_df = pd.DataFrame([low_latency_values, medium_latency_values, high_latency_values])
        sats_results_df['num sats'] = num_sats
        # print(sats_results_df) 
        if results_df is None:
            results_df = sats_results_df    
        else:
            results_df = pd.concat([results_df, sats_results_df], ignore_index=True)

        x = 1 # breakpoint for debugging    

    assert results_df is not None
    results_output_path = os.path.join(output_dir, f"latency_results_{trial_stem}.csv")
