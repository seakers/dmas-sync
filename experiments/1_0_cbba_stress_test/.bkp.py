...
def main_serialized(trial_filename : str, 
         lower_bound : int, 
         upper_bound : int, 
         level : int, 
         propagate_only : bool,
    ...
    # set simulation duration and step size
    duration = 25000 / 3600 / 24.0 if debug else 1.0 # [days]
    duration = min(duration, 1.0)                   # cap at 1 day for sanity
    step_size = 10                                  # [s]
...
if __name__ == "__main__":
    ...
    if debug or single_threaded or upper_bound - lower_bound <= 1:
        ...
        main_serialized(trial_filename, lower_bound, upper_bound, level, propagate_only, overwrite, evaluate, debug, runtime_profiling)
    else:
