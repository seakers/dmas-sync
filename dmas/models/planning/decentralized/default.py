...
class FixedPointingDefaultPlanner(AbstractReactivePlanner):
    ...
    def update_percepts(self, 
        ...
        if incoming_reqs:
            ...
            # if a task has been requested by anyone, remove it from the prescheduled list
            ...
            for req in incoming_reqs:
                if isinstance(req.task, EventObservationTask):
                    to_remove = {r for r in self._prescheduled_requests
                                 if isinstance(r.task, EventObservationTask)
                                 and r.task.event == req.task.event}
                else:
                    to_remove = {r for r in self._prescheduled_requests if r.task == req.task}

                if to_remove:
                    self._prescheduled_requests -= to_remove
                    self._future_tasks -= {r.task for r in to_remove}
    ...
    def generate_plan(  self, 
        ...
        try:
            ...
            broadcasts = []
            if self._enable_broadcasts:
                # task IDs we are still the designated announcer for (not yet claimed by others)
                prescheduled_task_ids : Set[str] = {req.task.id for req in self._prescheduled_requests}

                # collect tasks to announce:
                tasks_to_announce : Set[str] = set()
                # proactive — tasks already in the observation plan that we should announce
                # for obs, _ in observation_sequence:
                #     for task in obs.tasks:
                #         if task.id in prescheduled_task_ids:
                #             tasks_to_announce.add(task.id)

                # reactive — self-generated requests sitting in the outbox
                for req in self._outbox:
                    tasks_to_announce.add(req.task.id)
                    # if req.task.id in prescheduled_task_ids:
                        # tasks_to_announce.add(req.task.id)

                if tasks_to_announce:
                    # walk prescheduled broadcasts and keep only future messages for relevant tasks
                    t_broadcast_msgs : Dict[float, list] = defaultdict(list)
                    for broadcast_action in self._prescheduled_broadcasts:
                        if broadcast_action.t_start < t_curr - self.EPS:
                            continue  # skip broadcasts whose window has already passed

                        for msg in broadcast_action.msg.get('msgs', []):
                            if isinstance(msg, MeasurementRequestMessage):
                                if msg.req['task']['id'] in tasks_to_announce:
                                    t_broadcast_msgs[broadcast_action.t_start].append(msg)

                    # compile accumulated messages into one action per broadcast time
                    for t_broadcast, msgs in t_broadcast_msgs.items():
