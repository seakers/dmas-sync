...
class NadirPointingPeriodicPlanner(EarliestAccessPeriodicPlanner):
    ...
    def _schedule_maneuvers(    self, 
        ...
        # TEMPORARY ASSERTION
        assert state.attitude[0] <= self.EPS, f'Agent `{state.agent_name}` is not nadir pointing at time {state._t}. Current attitude: {state.attitude}'

        # Check if state is nadir pointing throughout the simulation
        if state.attitude[0] > self.EPS:
...
class NadirPointingReactivePlanner(EarliestAccessReactivePlanner):
    """ Only points agents in the downward direction """
    ...
    def _schedule_observations(self, 
        ...
        for obs in tqdm(sorted_observation_opportunities,
                         desc=f'{state.agent_name}-PLANNER: Pre-Scheduling Observations', 
                         leave=False,
